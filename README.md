...
### В папке configs_registry создать файл docker-compose.yml с конфигурациями контейнера
```
...
```
### Изменить права доступа к файлу entrypoint.sh
```
...
| REGISTRY_URL        | http://127.0.0.1          | URL, по которому доступен configs_registry                                    |
| REGISTRY_PORT       | 8002             | Порт, по которому доступен configs_registry                                   |
...
### В папке configs_service создать файл docker-compose.yml с конфигурациями контейнера
```
touch docker-compose.yml
```
```
...
```
Примечание: 
network_mode: "host" нужен, если взаимодействие с configs_registry происходит через localhost, а если по внешнему url - лучше поменять на порты
...
| BASE_URL             | https://api.beta.raida-dev.ru     | URL для запросов в Raida API                                                  |
| USER_RAIDA         | raida@raida.com    | Имя пользователя для запросов в Raida API                                     |
| PASSWD_RAIDA             | raida        | Пароль пользователя для запросов в Raida API                                  |
| ACCESS_TOKEN_PUBLIC_KEY         | public_key    | Публичный ключ для расшифровки JWT-токенов, полученных в Центре пользователей |
| JWT_ALGORITHM             | RS256        | Алгоритм шифрования JWT-токенов в Центре пользователей                        |
| CONFIGS_SERVICE_URL             | https://cfg.step.skroy.ru/        | URL, по которому доступен configs_service                                     |
| LANGUAGE_CODE         | ru    | Основной язык сервиса                                                         |
| TIME_ZONE             | Europe/Moscow        | Часовой пояс сервиса                                                          |

### В папке step_backend создать файл docker-compose.yml с конфигурациями контейнера (в файле можно поменять порты, на которых работает сервис)
```
touch docker-compose.yml
```
```
...
Работа всех методов сервиса step зависит от наличия в реестре необходимого набора конфигов, которые используются для взаимодействия с Raida API. В случае использования тестового Raida API набор конфигов должен быть следующим:
```
"task_status_id": {
  ...
  ```
Для других проектов значения могут меняться, но структура данных и названия должны оставаться прежними, иначе сервис будет выдавать ошибку:

{"detail":{"code":"SERVICE_ERROR","message":"Неправильный формат конфигураций для проекта"}}

Конфиги добавляются через метод POST /configs/ сервиса configs_service после авторизации с помощью JWT-токена, полученного в ЦП.

Запросы для добавления конфигов для тестовой Райды (нужно поменять URL и JWT-токен):

```
curl -X 'POST' \
  'http://127.0.0.1:8001/configs/' \
  -H 'Content-Type: application/json' \
  -H 'Project-ID: e3eb60a2-3bb7-4983-8e2f-47490b979eb9' \
  -H 'Authorization: Bearer <valid_jwt_token>' \
  -d '{
  "project_id": "e3eb60a2-3bb7-4983-8e2f-47490b979eb9",
  "object_type": "node_id",
  "data": {
    "value": "4fc9986b-d03b-4801-a672-a191c941e17c"
  }
...
curl -X 'POST' \
  ...
  -d '{
  "project_id": "e3eb60a2-3bb7-4983-8e2f-47490b979eb9",
  "object_type": "contest_process_id",
  "data": {
	  "value": "9e57ac56-9ce8-43fe-a725-d6eb6cb3758b"
  }
}'
